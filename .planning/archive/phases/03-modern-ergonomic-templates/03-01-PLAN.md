...
must_haves:
  truths:
    ...
    - "When a field declares type=string, emitted JSON preserves the original captured value as a JSON string (even if it looks numeric)"
    - "When conversion fails, the original captured value is preserved as a string"
    - "Heuristic numeric conversion upgrades obviously-numeric strings to JSON numbers when no explicit type is set (primarily legacy/untyped values)"
  artifacts:
...
- Conversion rules (must match Phase 03 decisions):
  - Explicit type hint wins.
    - REQUIRED in this phase: implement at least `FieldType::Int` and `FieldType::String`.
      - `FieldType::String` must be a no-op that preserves the raw string (disables heuristics).
...
- Add unit tests covering:
  ...
  - Explicit string conversion overrides heuristics: with `hint=Some(FieldType::String)`, raw like `"1,234"` stays a JSON string.
