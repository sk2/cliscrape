...
- Testing approach (make explicit so this is actually covered):
  - Add dev-dependency `assert_cmd` and write an integration test that runs the `cliscrape` binary with `--template-format yaml` against a template path with an unknown/ambiguous extension and asserts success.
  </action>
...
- Add integration tests that:
  ...
  - Run the CLI binary at least once via `assert_cmd` to prove `--template-format` override works independent of extension inference.
