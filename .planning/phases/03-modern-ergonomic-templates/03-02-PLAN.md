...
must_haves:
  truths:
    - "CLI parsing works on raw Cisco IOS-style transcripts that include prompts and command echoes"
...
<objective>
Add conservative Cisco IOS prompt + command-echo handling for raw CLI transcripts, and segment multi-command transcripts into per-command blocks before parsing.
...
- Confidence rules (must be conservative): only treat input as a transcript if prompt-like lines are observed at least twice with a stable hostname base; otherwise return the original input as a single block.
...
<verification>
- `cargo test`
- (Optional manual) Run: `cargo run -- parse -t templates/legacy/example.textfsm transcript.txt` and confirm prompts/echoes are ignored when present.
</verification>
