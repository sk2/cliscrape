...
must_haves:
  truths:
    ...
    - "Template-local macros override builtins and macro expansion is deterministic"
  artifacts:
    ...
    - path: src/engine/macros.rs
      provides: "Macro expansion supports recursion/cycle detection"
  key_links:
...
- Provide a `load_*` function that returns a lowered `TemplateIR` suitable for `Template::from_ir`.
  ...
  <done>Modern templates deserialize with strict unknown-key errors and can be lowered into `TemplateIR`.</done>
</task>
...
<task type="auto">
  <name>Task 3: Load modern templates by extension in FsmParser</name>
...
- Macro semantics match Phase 03 decisions (local overrides + recursion + cycle protection).
</success_criteria>
